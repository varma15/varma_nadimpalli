...
#Important features in random forest are:
#[1] "X0.32"
#[1] "X0.3"
#[1] "X0.32.1"
#[1] "X1.93"
...
```
Standardise data with important features from random forest.
```{r}
train_datanew=subset(train_data,select=c(X0.32,X0.3,X0.32.1,X1.93,X0.96,X0.15,X0.16,X0.17,X0.18,X0.38,X0.778,X0.44,X3.756,X61,X278))
test_datanew=subset(test_data,select=c(X0.32,X0.3,X0.32.1,X1.93,X0.96,X0.15,X0.16,X0.17,X0.18,X0.38,X0.778,X0.44,X3.756,X61,X278))
...
library(mxnet)
train.x = data.matrix(train_datanew1[,-16])
train.y = as.numeric(as.character(train_datanew1[,16]))
test.x = data.matrix(test_datanew1[,-16])
test.y = as.numeric(as.character(test_datanew1[,16]))
mx.set.seed(98)
Sys.time() -> start
