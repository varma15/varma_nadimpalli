...
library(caret)
x = nearZeroVar(spamdata, saveMetrics = TRUE)
y<-x[x[,"zeroVar"] + x[,"nzv"] > 0, ]
y<-t(y)
l<-colnames(y)
spamdata<-spamdata[,!(names(spamdata) %in% l)]
data(spamdata)
uni.plot(spamdata[,1:5])
spamdata2<-subset(spamdata1,select=c(48:57))
