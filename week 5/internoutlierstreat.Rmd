...
#standardise data
```{r}
train_datanew=subset(train_data,select=-c(X1,X0.23))
test_datanew=subset(test_data,select=-c(X1,X0.23))
...
library(dummies)
library(xgboost)
xgmodel<-xgboost(data=as.matrix(train_datanew1[,-57]),label=as.numeric(train_datanew1$X1),nrounds=500,eta=.01,gamma=1,subsample=.5,max_depth=1500)
y_pred<-predict(xgmodel,newdata=as.matrix(train_datanew1[,-57]))
