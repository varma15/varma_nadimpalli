...
m_dist <- mahalanobis(spamdata[,54:58], colMeans(spamdata[,54:58]), cov(spamdata[,54:58]))
spamdata$MD <- round(m_dist, 1)
spamdata$outlier<- ifelse(spamdata$MD > 12, "Yes","No")    # Threshold set to 20
...
model_gbm1
model_gbm <-gbm(X1 ~ . , cv.folds = 10, interaction.depth = 2,shrinkage =0.005,distribution='bernoulli',data=train_datanew1, n.trees = 1500)
model_gbm2 <-gbm(X1 ~ . , cv.folds = 10, interaction.depth = 3,shrinkage =0.0205,distribution='bernoulli',data=train_datanew1, n.trees = 1500)
