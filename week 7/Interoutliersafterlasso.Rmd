...
#standardise data
```{r}
train_datanew=subset(train_data,select=-c(X1,X0.64))
test_datanew=subset(test_data,select=-c(X1,X0.64))
...
library(mxnet)
train.x = data.matrix(train_datanew1[,-57])
train.y = as.numeric(as.character(train_datanew1[,57]))
test.x = data.matrix(test_datanew1[,-57])
test.y = as.numeric(as.character(test_datanew1[,57]))
mx.set.seed(98)
Sys.time() -> start
...
#model_knncross
plot(model_knncross)
model_knn <- knn3(X1 ~ . , train_datanew1, k =5)
...
model_gbm1
model_gbm <-gbm(X1 ~ . , cv.folds = 10, interaction.depth = 2,shrinkage =0.205,distribution='bernoulli',data=train_datanew1, n.trees = 1500)
...
library(dummies)
library(xgboost)
xgmodel<-xgboost(data=as.matrix(train_datanew1[,-57]),label=as.numeric(train_datanew1$X1),nrounds=200)
y_pred<-predict(xgmodel,newdata=as.matrix(test_datanew1[,-57]))
...
```
#Include decesion trees in stACKING AND SVM ON TOP
```{r}
train_preds_df <- data.frame(svm = preds_svmtrain,rf =preds_train_rf,C5 = preds_c5new,tree = preds_train_rpart, tree_bag = preds_train_tree_bag,gbm = preds_train_g2, X1 = train_datanew1$X1)
View(data.frame(preds_c5new))
# Getting all the predictions from the validation data into a dataframe.
test_preds_df<- data.frame(svm=preds_svm,rf = preds_rfvaltest,C5 = preds_c5new1,tree =preds_train_dec1, tree_bag = preds_tree_bag,gbm=preds_test_g2,X1 = test_datanew1$X1)
stack_df <- rbind(train_preds_df)

stack_df$X1 <- as.factor(stack_df$X1)
numeric_st_df <- sapply(stack_df[, !(names(stack_df) %in% "X1")],function(x) as.numeric(as.character(x)))
pca_stack <- prcomp(numeric_st_df, scale = F)
predicted_stack <- as.data.frame(predict(pca_stack, numeric_st_df))
stacked_df <- data.frame(predicted_stack, X1 = stack_df[, (names(stack_df) %in% "X1")])
stacked_model <- svm(X1 ~ . , stacked_df, kernel = "linear")
#stacked_model <- randomForest(X1 ~ . , stacked_df, ntrees=1500)
#stacked_model<-gbm(X1 ~ . , cv.folds = 20, interaction.depth = 10,shrinkage =0.0205,distribution='bernoulli',data=stacked_df,n.trees = 1500)
...
numeric_st_df_test <- sapply(test_preds_df[, !(names(test_preds_df) %in% "X1")],
                             function(x) as.numeric(as.character(x)))
predicted_stack_test <- as.data.frame(predict(pca_stack, numeric_st_df_test))
...
```
#Include decesion trees in stACKING AND rf ON TOP
...
```
#Include decesion trees in stacking AND gbm ON TOP
...
stacked_df <- data.frame(predicted_stack, X1 = stack_df[, (names(stack_df) %in% "X1")])
#stacked_model <- svm(X1 ~ . , stacked_df, kernel = "linear")
stacked_model <- gbm(X1 ~ . , cv.folds =5,interaction.depth = 10,shrinkage =0.0205,distribution='bernoulli',data=stacked_df,n.trees = 1500)
test_preds_df$X1 <- as.factor(test_preds_df$X1)
numeric_st_df_test <- sapply(test_preds_df[, !(names(test_preds_df) %in% "X1")],
                             function(x) as.numeric(as.character(x)))
predicted_stack_test <- as.data.frame(predict(pca_stack, numeric_st_df_test))[1:7]
