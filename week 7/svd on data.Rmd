...
std_method <- preProcess(spamdata,method = c("center","scale"))
spamdata <- predict(std_method, spamdata)
str(spamdata)
```
...
# with first 51 values itself, almost 95% is covered hence, 51 dimensions are enough
svd <- svd(spamdata[,-58],nu=52,nv=52)
S <- diag(svd$d[1:52])
u <- svd$u
...
spamdata1<-u
spamdata<-data.frame(cbind(spamdata1,X1=as.factor(spamdata$X1)))
str(spamdata)
spamdata$X1<-as.factor(spamdata$X1)
str(spamdata)
...
train_datanew1 <- spamdata[train_rows, ]
test_datanew1 <- spamdata[-train_rows, ]
train_datanew1$X1<-ifelse(train_datanew1$X1==2,1,0)
test_datanew1$X1<-ifelse(test_datanew1$X1==2,1,0)
str(train_datanew1$X1)
train_datanew1$X1<-as.factor(train_datanew1$X1)
test_datanew1$X1<-as.factor(test_datanew1$X1)
...
library(mxnet)
train.x = data.matrix(train_datanew1[,-53])
train.y = as.numeric(as.character(train_datanew1[,53]))
test.x = data.matrix(test_datanew1[,-53])
test.y = as.numeric(as.character(test_datanew1[,53]))
mx.set.seed(98)
Sys.time() -> start
...
model_gbm1
model_gbm <-gbm(X1 ~ . , cv.folds = 10, interaction.depth = 2,shrinkage =0.005,distribution='bernoulli',data=train_datanew1, n.trees = 1500,bag.fraction = 0.1)
model_gbm2 <-gbm(X1 ~ . , cv.folds = 10, interaction.depth = 3,shrinkage =0.0205,distribution='bernoulli',data=train_datanew1, n.trees = 1500,bag.fraction = 0.1)
...
library(dummies)
library(xgboost)
xgmodel<-xgboost(data=as.matrix(train_datanew1[,-53]),label=as.numeric(train_datanew1$X1),nrounds=200)
y_pred<-predict(xgmodel,newdata=as.matrix(test_datanew1[,-53]))
y_pred1<-ifelse(y_pred>1.5,1,0)
confusionMatrix(y_pred1,test_datanew1$X1)

```
C 5.0
```{r}
library(C50)
c5_tree<-C5.0(X1~.,train_datanew1,cp=100,ntrees=1500)
...
```{r}
train_preds_df <- data.frame(svm = preds_svmtrain,rf =preds_train_rf,knn = preds_c5new,tree = preds_train_rpart, tree_bag = preds_train_tree_bag,gbm = preds_train_g2, X1 = train_datanew1$X1)
View(data.frame(preds_c5new))
# Getting all the predictions from the validation data into a dataframe.
test_preds_df<- data.frame(svm=preds_svm,rf = preds_rfvaltest,knn = preds_c5new1,tree =preds_train_dec1, tree_bag = preds_tree_bag,gbm=preds_test_g2,X1 = test_datanew1$X1)
stack_df <- rbind(train_preds_df)
