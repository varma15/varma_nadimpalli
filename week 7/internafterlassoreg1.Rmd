...
model_svm <- svm(X1 ~ . , train_datanew1, kernel = "linear",class.weights = c('0'=1,'1'=1.95))
...
model_rf <- randomForest(X1 ~ . , train_datanew1,ntrees=1500,sampsize=.7*nrow(train_datanew1),.mtry=7)
varImpPlot(model_rf)
...
model_gbm1
model_gbm <-gbm(X1 ~ . , cv.folds = 10, interaction.depth = 10,shrinkage =0.001,distribution='bernoulli',data=train_datanew1, n.trees = 1500,bag.fraction = 0.1)
gbm.perf(model_gbm)
